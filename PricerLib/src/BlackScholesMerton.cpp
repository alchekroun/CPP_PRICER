...
double pricer::bsm::get_option_price(bool const is_call, double const spot, double const strike, double const rate, double const volatility, double const maturity, double const dividend) {
	...
	if (is_call) {
		return spot * exp(-dividend * maturity) * normal_cdf(d1_d2.first) - strike * exp(-rate * maturity) * normal_cdf(d1_d2.second);
	}
	return strike * exp(-rate * maturity) * normal_cdf(-d1_d2.second) - spot * exp(-dividend * maturity) * normal_cdf(-d1_d2.first);
}
...
double pricer::bsm::get_theta(bool const is_call, double const spot, double const strike, double const rate, double const volatility, double const maturity, double const dividend) {
	...
	return -((spot * normal_pdf(d1_d2.first) * volatility * exp(-dividend * maturity)) / (2 * sqrt(maturity))) + (dividend * spot * normal_cdf(d1_d2.first) * exp(-dividend * maturity)) - (rate * strike * exp(-rate * maturity) * normal_cdf(d2_used));
}
